...
namespace sf
{
    ...
    {
        GLint windowW;              //!< 窗口宽度（像素）
        GLint windowH;              //!< 窗口高度（像素）
        RenderQuality shadows;      //!< 阴影质量
        RenderQuality ao;           //!< 环境光遮蔽（Ambient Occlusion）质量
        RenderQuality atmosphere;   //!< 大气渲染质量（天空、太阳等）
        RenderQuality ocean;        //!< 海洋渲染质量（波浪、水面反射等）
        RenderQuality aa;           //!< 抗锯齿（Anti-Aliasing）质量
        RenderQuality ssr;          //!< 屏幕空间反射（Screen Space Reflection）质量
        bool verticalSync;          //!< 垂直同步开关（防止画面撕裂）
