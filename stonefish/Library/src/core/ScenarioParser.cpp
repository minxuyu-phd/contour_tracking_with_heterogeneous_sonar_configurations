...
Sensor* ScenarioParser::ParseSensor(XMLElement* element, const std::string& namePrefix)
{
    ...
    else if(typeStr == "echosounder")
    {
        int history;
        Scalar beamWidth = Scalar(10);  // Default beam width 10 degrees
        bool positiveZ = false;         // Default: beam points downward

        if((item = element->FirstChildElement("history")) == nullptr || item->QueryAttribute("samples", &history) != XML_SUCCESS)
            history = -1;

        // Optional specs definition
        if((item = element->FirstChildElement("specs")) != nullptr)
        {
            item->QueryAttribute("beam_width", &beamWidth);
            item->QueryAttribute("positive_z", &positiveZ);
        }

        EchoSounder* es = new EchoSounder(sensorName, beamWidth, positiveZ, rate, history);
