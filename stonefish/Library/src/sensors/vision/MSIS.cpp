...
MSIS::MSIS(std::string uniqueName,
           Scalar stepAngleDeg,
...
void MSIS::setRotationLimits(Scalar l1Deg, Scalar l2Deg)
{
    ...
    GLint newRoiX = (GLint)round(btRadians(l1Deg)/stepSize);
    GLint newRoiY = (GLint)round(btRadians(l2Deg)/stepSize);
    bool newFullRotation = (newRoiX == -(int)resX/2) && (newRoiY == (int)resX/2);
    if(newRoiY == (int)resX/2)
        --newRoiY;

    // Update range parameters
    roi.x = newRoiX;
    roi.y = newRoiY;
    fullRotation = newFullRotation;

    // Check if current position is within new range
    if(currentStep >= roi.x && currentStep <= roi.y)
    {
        // Current position is within new range, keep currentStep and cw unchanged
    }
    else
    {
        // Current position is outside new range, reset to start position
        currentStep = roi.x;
        cw = true;
    }
}
...
void MSIS::setUnidirectional(bool enabled)
{
    unidirectional = enabled;
}

bool MSIS::isUnidirectional() const
{
    return unidirectional;
}

void MSIS::setStepMultiplier(int multiplier)
{
    if(multiplier >= 1 && multiplier <= 10)
        stepMultiplier = multiplier;
}

int MSIS::getStepMultiplier() const
{
...
void MSIS::NewDataReady(void* data, unsigned int index)
{
    ...
    if(index == 1)
    {
        currentStep += cw ? stepMultiplier : -stepMultiplier;

        if(fullRotation || unidirectional)
        {
